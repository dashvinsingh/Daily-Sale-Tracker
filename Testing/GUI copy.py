...
class GUI:
    def __init__(self, master):
        ...
        self.new_sale.config(image=self.photo1,font=("Century Gothic", 35,'bold'),height=h\
                             ,width = w,command=self.new_sale_func)
        ...
        self.delete_sale.config(image=self.photo1,font=font_but,height=h\
                             ,width = w, command=self.del_sale_func)
        ...
        self.recent.config(image=self.photo1,font=font_but,height=h\
                             ,width = w,command=self.find_sale_func)
...
##        Label(master, text = "Sale Count"
    ...
    def new_sale_func(self):
        messagebox.showinfo('New Sale', 'NEW SALE')
    def new_sale_key(self, event):
        self.new_sale_func()

    #Delete Sale
    def del_sale_func(self):
        messagebox.showinfo('Delete Sale', 'Delete SALE')
...
class _Authen:
    def __init__(self, master, gui_name):
        gui_dict={'main':GUI(root), 'current_sales':}
        frame_color='light blue'
